...
if __name__ == "__main__":
    ...
    for i in range(START,STOP): 
        ...
        if WRITE_JSON:
            output_dir = '/home/drussel1/data/ADL/openpose_json_keypoints/keypoint_{:02d}'.format(i)
        else:
            output_dir = '/home/drussel1/data/ADL/openpose_keypoints/keypoint_{:02d}'.format(i)

        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        keypoint_extractor = None # hopefully free the GPU memory
        if WRITE_JSON:
            keypoint_extractor = keypointExtractor(output_dir) # write to the output_dir
        else:
            keypoint_extractor = keypointExtractor(None) # write to the output_dir
